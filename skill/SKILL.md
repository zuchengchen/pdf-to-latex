...
9. Reconstruct by semantic region and structural boundary. Use page-bounded text-layer evidence only as evidence; correct it visually. Visually transcribe scanned, damaged, encoded, and complex regions. Parallel workers may produce bounded page-IR shards, but page boundaries are not semantic boundaries and workers must not edit shared final source or state. Keep genuine figures as project assets and rebuild legible tables and formulas semantically.
...
For batch-enabled projects, use `batch-manifest.json` as a supplemental machine-readable ledger. Store worker results under `work/shards/`, bind every shard to the source identity and current style/document-IR snapshot, and merge through `scripts/merge_shards.py`. The ledger does not replace the canonical Markdown records; it records ownership, attempts, hashes, and merge checkpoints so Goal prompts can remain concise.

...
11. **Clean-room build gate**: before delivery, rebuild from a clean project copy or clean working tree state so hidden auxiliary files, absolute paths, stale generated files, or missing assets are caught. Prefer `scripts/publication_gate.sh TARGET_DIR main.tex` when available, then record the result.
...
5. Run the production spec gate for `publication polish`, or a lighter style decision pass for other delivery levels. Record class, geometry, fonts, source structure, object strategy, bibliography/index/glossary strategy, toolchain assumptions, and non-goals before broad reconstruction.
...
18. Run final reviewer gates required for `publication polish`: structure/content review, math/object review, and build/layout review. Use subagents for bounded findings when permitted; otherwise perform separate passes and record results in `conversion-notes.md`.
19. Run the final quality review from `references/quality-review.md`, including the clean-room build gate. Prefer `scripts/publication_gate.sh TARGET_DIR main.tex` when available for deterministic compile, artifact, render, and clean rebuild checks.

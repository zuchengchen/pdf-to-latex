...
```text
Use $pdf-to-latex to rebuild SOURCE_PDF into an editable XeLaTeX project under TARGET_DIR at DELIVERY_LEVEL. Continue from TARGET_DIR/conversion-state.md on every turn when it exists. Complete only when initial triage is complete, the exact task profile (`light`, `standard`, `book`, `math-heavy`, or `book-math`) and delivery level are recorded, any profile upgrade is documented, the production spec gate is complete for publication polish, durable page evidence exists under TARGET_DIR/evidence/source-pages when visual transcription or comparison is needed, page-bounded text-layer evidence exists under TARGET_DIR/evidence/text-layer when digital extraction is used, page-manifest.md records any long-document batch plan and page or region routes, page-manifest.md and page transcripts exist when the selected profile requires them, object-inventory.md, style-profile.md, and document-ir.md exist or their light-profile omission is documented, book-production checks are completed when the document is a book, thesis, monograph, proceedings volume, or has front/back matter, math-inventory.md and glyph-map.md exist when the document is math-heavy or encoded, the skeleton compile gate has passed before broad final drafting for publication polish, main.tex and chapter files are generated from document-ir.md or a documented light-profile outline, high-risk batches compile before moving on, assets and bibliography/index/glossary inputs are present or documented as unresolved, the project compiles successfully with XeLaTeX, minimum refinement passes and reviewer gates are completed according to DELIVERY_LEVEL, math artifact scans of final source are clean when applicable, the clean-room build gate passes for publication polish, quality-review.md checks pass or true blockers are documented, conversion-notes.md records verification, and conversion-state.md says Next action: None; quality review complete.
```
...
- For publication polish, `style-profile.md` and `conversion-notes.md` record the production spec.
...
- The skeleton compile gate passed before broad final drafting when publication polish was selected.
...
- Publication-polish reviewer gates for structure/content, math/object, and build/layout are complete when applicable.

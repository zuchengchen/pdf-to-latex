...
def parse_openai_yaml(path: Path) -> tuple[dict[str, str], list[str]]:
    ...
    try:
        ...
        return {}, [f"Cannot read UTF-8 agents/openai.yaml: {exc}"]
    ...
    for line_number, line in enumerate(lines[1:], start=2):
        ...
        if not match:
            errors.append(f"agents/openai.yaml:{line_number}: unsupported YAML structure.")
            continue
        key, scalar = match.groups()
        if key in values:
            errors.append(f"agents/openai.yaml:{line_number}: duplicate interface field: {key}")
            continue
        try:
            value = json.loads(scalar)
        except json.JSONDecodeError:
            errors.append(
                f"agents/openai.yaml:{line_number}: interface values must be double-quoted strings."
            )
            continue
        if not isinstance(value, str) or not value.strip():
            errors.append(f"agents/openai.yaml:{line_number}: interface value must be non-empty.")
            continue
        values[key] = value
